...
# lesson titles for each level retrieved from each level html file using js
### get_titles.JS ###
# titleList = []
# $('.linkList a h2').each((index, item) => titleList.push(item.innerText))
# copy(JSON.stringify(titleList))
#####################
...
# Download lessons mp3 files for passed lvl
def download_course_lvl(base_url, lvl, titles_list):
  
  # customize url for requested lvl
  ...
  # Download each of the 26 lessons in the level
  for i in range(1, 27):
    lesson_url = lvl_url
    if i < 10:
        lesson_url += '0' + str(i)
...
# base url format of an mp3 lesson in "why not?"
